...
_spec = importlib.util.spec_from_file_location("v269_corrected_systematic_tail_base", V269_SOURCE)
...
def self_test() -> None:
    ...
    print("V325-V332 causal systematic-tail self-test passed")
...
def stricter_postprocess(root: Path) -> None:
    ...
    proof["coverage_gate"]["candidate"] = "V325_FIXED_UNIVERSE_DATA_COVERAGE"
    proof["design_sha256"] = hashlib.sha256(
        (root / "V325_V332_DESIGN.json").read_bytes()
    ).hexdigest()
    ...
    summary["coverage_gate"]["candidate"] = "V325_FIXED_UNIVERSE_DATA_COVERAGE"
    summary["limitations"] = [
        ...
        "V325 corrects a double-warmup arithmetic defect before any V325/V317 OOS was opened.",
    ...
    if selected is None:
        report = (
            "# Active V325–V332 — corrected crypto systematic tail\n\n"
...
def run(root: Path, cache: Path) -> int:
    ...
    alias.write_bytes((root / "V325_V332_DESIGN.json").read_bytes())
    try:
