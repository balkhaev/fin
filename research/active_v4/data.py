...
def download_symbol(symbol: str, config: ResearchConfig, cache: Path, refresh: bool = False):
    ...
    for month in iter_months(config.data_start, config.end_exclusive):
        next_month = month + pd.offsets.MonthBegin(1)
        clip_start, clip_end = max(month, utc(config.data_start)), min(next_month, end)
...
def validate_daily(frame: pd.DataFrame, symbol: str) -> dict[str, object]:
    ...
    return {
        ...
        "largest_gap_days": float(diffs.max().total_seconds() / 86_400),
...
def load_all(config: ResearchConfig, cache: Path, refresh: bool = False):
    ...
    for symbol in config.symbols:
        ...
        quality.append(validate_daily(frame, symbol))
        daily[symbol] = frame
        records.extend(manifest)
        print(f"{symbol}: {len(frame):,} daily bars")
