...
def _funding_blocks(
    ...
    if frame.empty:
        return pd.DataFrame(
            columns=["timestamp", f"funding_{venue}", f"funding_count_{venue}"]
        )
    values = frame.copy()
    values["timestamp"] = pd.to_datetime(values["timestamp"], utc=True)
    # A payment stamped exactly at a boundary belongs to the holding interval
    # that just ended.  Subtracting one nanosecond makes the replay causal for
    # entries executed at the boundary after the payment.
    values["block"] = (
        values.timestamp - pd.Timedelta(nanoseconds=1)
    ).dt.floor(f"{BLOCK_HOURS}h")
    grouped = values.groupby("block", as_index=False).agg(
        rate=("rate", "sum"), count=("rate", "count")
    )
