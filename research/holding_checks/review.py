"""Read-only audit of existing saved ledgers. Does not simulate, repair or trade."""
...
def review(source: Path, reports: Path, out: Path) -> dict:
    ...
    if digest(result) != proof['result_sha256'] or len(result['rows']) != 68:
        raise ValueError('Original calculated result changed')
    modules, frames, audit, _, pins = load_reference(source, acknowledged=True)
    if audit != result['source'] or pins != result['original_reference_pins']:
        raise ValueError('Audit source or reference mismatch')
    close = pd.DataFrame({s: frames[s].close for s in ('BTCUSDT', 'ETHUSDT')})
    target = candidates(close, modules['opportunity_runner.study'].states)
    ...
    for r in result['rows']:
        ...
        for symbol in ('BTCUSDT', 'ETHUSDT'):
            ...
            if len(own):
                ...
                if not (own.quantity_delta.abs().to_numpy() <= cap + 1e-8).all():
                    raise AssertionError('Fill exceeded declared previous-hour capacity')
            ...
            quantity = np.zeros(len(schedule))
            valid = before >= 0
            if len(cumulative):
                quantity[valid] = cumulative[before[valid]]
            ...
            expected = -quantity[held] * schedule.mark_open.to_numpy()[held] * schedule.funding_rate.to_numpy()[held]
            np.testing.assert_allclose(got.cashflow, expected, rtol=1e-9, atol=1e-7)
            np.testing.assert_allclose(got.quantity, quantity[held], rtol=0, atol=1e-8)
            if r['accounting_complete'] and abs(changes.sum()) > 1e-8:
                raise AssertionError('Complete account retains signed contracts')
        groups = list(f[f.reason == 'entry'].groupby('time', sort=True))
        if r['accounting_complete'] and len(groups) != len(ep):
            raise AssertionError('Completed account entries/episodes disagree')
        ...
        for n, (when, group) in enumerate(groups):
            ...
            if n < len(ep):
                previous_cash = float(ep.iloc[n].end_balance)
        ...
        checks.append({'model': r['model'], 'period': r['period'], 'start': r['start'],
            'fill_rows': len(f), 'funding_rows': len(pay), 'entry_groups': len(groups)})
    evidence = dict(result_sha256=proof['result_sha256'], reports=68, fill_rows=count_fills,
        ...
        original_source_not_modified=True, no_new_account_simulation=True, actual_exchange_execution_not_verified=True,
        rows=checks)
...
if __name__ == '__main__':
    ...
    p.add_argument('--source', type=Path, required=True); p.add_argument('--reports', type=Path, required=True)
