...
_spec = importlib.util.spec_from_file_location("v269_residual_reversal_base", V269_SOURCE)
...
def stricter_postprocess(root: Path) -> None:
    ...
    proof["coverage_gate"]["candidate"] = "V293_FIXED_UNIVERSE_DATA_COVERAGE"
    proof["design_sha256"] = hashlib.sha256(
        (root / "V293_V300_DESIGN.json").read_bytes()
    ).hexdigest()
    ...
    summary["coverage_gate"]["candidate"] = "V293_FIXED_UNIVERSE_DATA_COVERAGE"
    summary["limitations"] = [
    ...
    if selected is None:
        report = (
            "# Active V293–V300 — crypto residual reversal\n\n"
...
def run(root: Path, cache: Path) -> int:
    ...
    alias.write_bytes((root / "V293_V300_DESIGN.json").read_bytes())
    try:
