...
_spec = importlib.util.spec_from_file_location("v269_systematic_tail_base", V269_SOURCE)
...
def stricter_postprocess(root: Path) -> None:
    ...
    proof["coverage_gate"]["candidate"] = "V301_FIXED_UNIVERSE_DATA_COVERAGE"
    proof["design_sha256"] = hashlib.sha256(
        (root / "V301_V308_DESIGN.json").read_bytes()
    ).hexdigest()
    ...
    summary["coverage_gate"]["candidate"] = "V301_FIXED_UNIVERSE_DATA_COVERAGE"
    summary["limitations"] = [
    ...
    if selected is None:
        report = (
            "# Active V301–V308 — crypto systematic tail\n\n"
...
def run(root: Path, cache: Path) -> int:
    ...
    alias.write_bytes((root / "V301_V308_DESIGN.json").read_bytes())
    try:
