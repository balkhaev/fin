...
CANDIDATE = "ACTIVE_V341_CRYPTO_VOLATILITY_TERM_STRUCTURE"
FAMILIES = (
...
def self_test() -> None:
    ...
    policy = v.Policy("low_residual_vol_ratio", 30, 3, 14, "dollar")
    ...
    print("V341-V348 causal volatility-term-structure self-test passed")
...
def stricter_postprocess(root: Path) -> None:
    ...
    summary["coverage_gate"]["candidate"] = "V341_FIXED_UNIVERSE_DATA_COVERAGE"
    summary["limitations"] = [
        ...
        "Volatility compression is a statistical ranking, not an issuer-quality claim.",
    ]
    ...
    if selected is None:
        report = (
            "# Active V341–V348 — volatility term structure\n\n"
...
def run(root: Path, cache: Path) -> int:
    ...
    alias.write_bytes((root / "V341_V348_DESIGN.json").read_bytes())
    try:
