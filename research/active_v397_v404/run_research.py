...
CANDIDATE = "ACTIVE_V397_CRYPTO_DISPERSION_SENSITIVITY"
FAMILIES = (
...
def self_test() -> None:
    ...
    policy = v.Policy("low_residual_dispersion_beta", 60, 3, 14, "dollar")
    ...
    print("V397-V404 causal dispersion-sensitivity self-test passed")
...
def stricter_postprocess(root: Path) -> None:
    ...
    proof["coverage_gate"]["candidate"] = "V397_FIXED_UNIVERSE_DATA_COVERAGE"
    proof["lookbacks"] = list(WINDOWS)
    proof["design_sha256"] = hashlib.sha256(
        (root / "V397_V404_DESIGN.json").read_bytes()
    ).hexdigest()
    ...
    summary["coverage_gate"]["candidate"] = "V397_FIXED_UNIVERSE_DATA_COVERAGE"
    summary["limitations"] = [
        ...
        "Dispersion sensitivity is a statistical market-linkage ranking, not an issuer-quality claim.",
    ]
    ...
    if selected is None:
        report = (
            "# Active V397–V404 — dispersion sensitivity\n\n"
...
def run(root: Path, cache: Path) -> int:
    ...
    alias.write_bytes((root / "V397_V404_DESIGN.json").read_bytes())
    try:
