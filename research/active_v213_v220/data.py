...
PERP_BASE = "https://data.binance.vision/data/futures/um/monthly/klines"
KLINE_COLUMNS = (
    "open_time",
    "open",
    "high",
...
def _session() -> requests.Session:
    ...
    client.headers.update(
        {
            "User-Agent": "fin-research-v213/1.0 (+https://github.com/balkhaev/fin)",
...
def parse_kline_zip(payload: bytes) -> pd.DataFrame:
    ...
    if frame.empty:
        return pd.DataFrame(
            columns=["timestamp", "open", "close", "quote_volume", "taker_buy_quote"]
        )
    if frame.shape[1] < 11:
...
def download_market(
    ...
    target = cache / f"{market}_{symbol}_{BAR_MINUTES}m.csv"
    metadata = cache / f"{market}_{symbol}_{BAR_MINUTES}m_provenance.json"
    ...
    client = _session()
    base = SPOT_BASE if market == "spot" else PERP_BASE
...
def _rename(frame: pd.DataFrame, suffix: str) -> pd.DataFrame:
    result = frame.copy()
    result = result.rename(
        columns={
            "open": f"open_{suffix}",
            "close": f"close_{suffix}",
            "quote_volume": f"quote_volume_{suffix}",
            "taker_buy_quote": f"taker_buy_quote_{suffix}",
        }
    )
    return result


def build_asset_frame(spot: pd.DataFrame, perp: pd.DataFrame) -> pd.DataFrame:
    start = pd.Timestamp(START, tz="UTC").floor(f"{BAR_MINUTES}min")
    end = pd.Timestamp(END, tz="UTC").floor(f"{BAR_MINUTES}min")
    index = pd.DataFrame(
        {
            "timestamp": pd.date_range(
                start, end, freq=f"{BAR_MINUTES}min", tz="UTC"
            )
        }
    )
    frame = index.merge(_rename(spot, "spot"), on="timestamp", how="left")
    frame = frame.merge(_rename(perp, "perp"), on="timestamp", how="left")
    for market in ("spot", "perp"):
        quote = pd.to_numeric(frame[f"quote_volume_{market}"], errors="coerce")
        buy = pd.to_numeric(frame[f"taker_buy_quote_{market}"], errors="coerce")
        frame[f"flow_{market}"] = np.where(
            quote > 0,
            2.0 * buy / quote - 1.0,
            np.nan,
        )
...
def load_all(cache: Path) -> tuple[dict[str, pd.DataFrame], dict[str, Any]]:
    markets: dict[str, pd.DataFrame] = {}
    provenance: dict[str, Any] = {"assets": {}}
    for asset in ASSETS:
        symbol = SYMBOLS[asset]
        print(f"loading {asset} spot/perp", flush=True)
        spot, spot_meta = download_market("spot", symbol, cache / "spot")
        perp, perp_meta = download_market("perp", symbol, cache / "perp")
        frame = build_asset_frame(spot, perp)
        markets[asset] = frame
        complete = frame.complete.astype(bool)
        provenance["assets"][asset] = {
            ...
            "complete_timestamp_min": (
                frame.loc[complete, "timestamp"].min().isoformat()
                if complete.any()
                else None
            ),
            "complete_timestamp_max": (
                frame.loc[complete, "timestamp"].max().isoformat()
                if complete.any()
                else None
            ),
        }
