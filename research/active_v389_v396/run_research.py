...
CANDIDATE = "ACTIVE_V389_CRYPTO_RESIDUAL_RESILIENCE"
FAMILIES = (
...
def _drawdown_duration(values: np.ndarray) -> float:
...
def self_test() -> None:
    ...
    policy = v.Policy("low_residual_drawdown_duration", 60, 3, 14, "dollar")
    ...
    print("V389-V396 causal residual-resilience self-test passed")
...
def stricter_postprocess(root: Path) -> None:
    ...
    proof["coverage_gate"]["candidate"] = "V389_FIXED_UNIVERSE_DATA_COVERAGE"
    proof["lookbacks"] = list(WINDOWS)
    proof["design_sha256"] = hashlib.sha256(
        (root / "V389_V396_DESIGN.json").read_bytes()
    ).hexdigest()
    ...
    summary["coverage_gate"]["candidate"] = "V389_FIXED_UNIVERSE_DATA_COVERAGE"
    summary["limitations"] = [
        ...
        "Residual resilience is a statistical path-recovery ranking, not an issuer-quality claim.",
    ]
    ...
    if selected is None:
        report = (
            "# Active V389–V396 — residual drawdown resilience\n\n"
...
def run(root: Path, cache: Path) -> int:
    ...
    alias.write_bytes((root / "V389_V396_DESIGN.json").read_bytes())
    try:
