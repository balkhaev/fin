...
_SPEC = importlib.util.spec_from_file_location("v341_correlation_harness", SOURCE)
...
CANDIDATE = "ACTIVE_V357_CRYPTO_CORRELATION_STABILITY"
FAMILIES = (
    "low_market_correlation",
    "stable_market_correlation",
    "stable_market_beta",
    "reversed_high_correlation_control",
)
...
def rolling_beta(
    ...
    variance = market_return.rolling(window, min_periods=window).var()
    output = pd.DataFrame(index=frame.index, columns=frame.columns, dtype=float)
    for symbol in frame.columns:
        covariance = frame[symbol].rolling(window, min_periods=window).cov(market_return)
        output[symbol] = covariance / variance.replace(0.0, np.nan)
    return output


def score_frame(market: Any, family: str, lookback: int) -> pd.DataFrame:
    correlation = rolling_correlation(market.logret, market.market, int(lookback))
    if family == "low_market_correlation":
        raw = -correlation
    elif family == "reversed_high_correlation_control":
        raw = correlation
    elif family == "stable_market_correlation":
        inner = rolling_correlation(market.logret, market.market, INNER_WINDOW)
        raw = -inner.rolling(int(lookback), min_periods=int(lookback)).std(ddof=1)
    elif family == "stable_market_beta":
        inner_beta = rolling_beta(market.logret, market.market, INNER_WINDOW)
        raw = -inner_beta.rolling(int(lookback), min_periods=int(lookback)).std(ddof=1)
    else:
...
def self_test() -> None:
    ...
    policy = h.v.Policy("stable_market_correlation", 60, 3, 14, "dollar")
    ...
    print("V357-V364 causal correlation-stability self-test passed")
...
def adjust_outputs(root: Path) -> None:
    ...
    proof["coverage_gate"]["candidate"] = "V357_FIXED_UNIVERSE_DATA_COVERAGE"
    proof.pop("window_pairs", None)
    proof["lookbacks"] = list(LOOKBACKS)
    proof["inner_estimation_window_days"] = INNER_WINDOW
    proof["design_sha256"] = hashlib.sha256(
        (root / "V357_V364_DESIGN.json").read_bytes()
    ).hexdigest()
    ...
    summary["coverage_gate"]["candidate"] = "V357_FIXED_UNIVERSE_DATA_COVERAGE"
    summary["limitations"] = [
        "Rolling correlation and beta are statistical estimates and may change abruptly between rebalances.",
    ...
    if selected is None:
        report = (
            "# Active V357–V364 — correlation stability\n\n"
...
def run(root: Path, cache: Path) -> int:
    ...
    alias.write_bytes((root / "V357_V364_DESIGN.json").read_bytes())
    try:
