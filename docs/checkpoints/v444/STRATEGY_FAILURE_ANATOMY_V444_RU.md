# V437–V444 — state-conditioned strategy failure anatomy
...
dollar P&L                          $-550.24
turnover                            3.254x
novel loss share                    8.9%
state-entry loss share              7.9%
...
dollar P&L                          $-1,960.76
turnover                            15.325x
novel loss share                    24.1%
state-entry loss share              7.5%
persistent state >5d loss share     65.5%
```

Largest negative states: rotation (-5.74%), transition (-2.71%), transition_2 (-1.61%).

## Общая структура

Common negative states: `deleveraging`.

Common negative transitions: `deleveraging->deleveraging`.
